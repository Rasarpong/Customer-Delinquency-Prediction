...
The aim of this project is to fit a logistic and MARS model using real accounts data from a credit card company in Sioux Falls, South Dakota for predicting if a customer is delinquent or not and determine which model performs best.  The concept of binning will be applied in this project. Binning is a way to group a number of more or less continuous values into a smaller number of “bins”. For example, if you have data about a group of people, you might want to arrange their ages into a smaller number of age intervals. The Multivariate adaptive regression splines (MARS) provide a convenient approach to capture the nonlinear relationships in the data by assessing cutpoints (knots) similar to step functions. The procedure assesses each data point for each predictor as a knot and creates a linear regression model with the candidate feature(s. Model comparison is done to compare the  predictive power of the models using Receiver Operator Characteristics curves(ROC) and the Kolmogorov-Smirnov (KS) statistic. 
...
Below is a summary of the 19 variables in the dataset. This does not include rows with missing values. The analysis that folows makes use of rows with complete data. rows with missing observations made up small portion (5.12%) of the total dataset therefore there were removed. Also, duplicated rows were removed from the dataset. The final dataset after the processing stage had 5916 observations with 18 predictors for customer delinquency.
...
Binning is a good idea when there are discontinuity in the variable at the cut-points. Some continuos predictor variables used for building models  were binned. Binning is a way to group a number of more or less continuous values into a smaller number of “bins”. These continous variables now are treated as factor/categorical variables. Below is the visualisation of some binned continous predictors.
...
The Kolmogorov-Smirnov (KS) statistic is a performance statistic which measures the discriminatory power of a model. It is the largest difference between the TPR and FPR at a given percentile. It looks at the maximum difference between the distribution of cumulative events and cumulative non-events. It is a very popular metric used in credit risk and response modeling. The Kolmogorov–Smirnov test a very eﬀicient way to determine if two samples are significantly different from each other. In predictive analytics, the test is used to determine if predictions from different models differ significantly from each other. **The higher the value, the better the model.**
...
CONCLUSION
------------------
The MARS and Logistic models are two good models in predicting customer deliquency. Even though the AUC of both models are equal, based on the KS statistic, the logistic model outperforms the MARS model since it has the highest value. 
